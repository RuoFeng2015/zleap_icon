...
## 最新更新 (2026-01-19 v2.4.0)

### 修复：全量覆盖模式真正删除旧文件

**问题**：即使使用全量覆盖模式，旧图标仍然保留在 main 分支
...
1. 从新创建的 PR 分支获取文件列表
2. 删除所有文件
3. 但新分支刚创建时就包含了所有旧文件（从 main 复制）
4. 删除操作在 PR 分支中执行，但 Git 合并时不会删除 main 分支上的文件
...
1. ✅ 从 main 分支获取文件列表（确保获取到所有旧文件）
2. ✅ 对比本次上传的文件名
3. ✅ 只删除不在上传列表中的文件
4. ✅ 保留本次上传的文件（即使它们已经存在）

**测试步骤**：

1. **重新加载 Figma 插件**（关闭并重新打开）
2. 选择 6 个新图标
3. 选择全量覆盖模式
4. 上传
5. 查看浏览器控制台日志（F12），应该看到：
   ```
   找到 43 个现有文件，本次上传 6 个文件
   需要删除 37 个旧文件
   ✅ 已删除: icon-1.svg
   ✅ 已删除: icon-2.svg
   ...
